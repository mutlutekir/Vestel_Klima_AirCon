...
[![Maintainer](https://img.shields.io/badge/maintainer-Mutlu%20Tekir-green?style=for-the-badge)](https://github.com/mutlutekir)
...
[vestel-ac-remote-control](https://github.com/iltekin/vestel-ac-remote-control)
...
1. HACS içinde **Integrations → sağ üst ⋮ → Custom repositories**.
2. Repo URL'sini ekle, kategori olarak **Integration** seç.
3. **"Vestel AC"**yi ara, indir.
4. Home Assistant'ı **yeniden başlat**.

### Yöntem 2: Manuel

1. `custom_components/vestel_ac/` klasörünü HA config dizinindeki
   `custom_components/` altına kopyala.
2. Home Assistant'ı **yeniden başlat**.

### ⚙️ Yapılandırma

1. **Ayarlar → Cihazlar ve Hizmetler → + Entegrasyon Ekle → "Vestel AC"**.
2. **"Kullanıcı adı / şifre (otomatik)"** seçeneğini seç, Vestel Akıllı
...
- `vestel_ac.dump_raw_status` — cihazın o anki tüm ham alanlarını bildirim
  olarak gösterir.
- `vestel_ac.send_raw_code` — elle yazdığın bir kodu (`{"code":
  "ACFANPO00562"}` gibi) doğrudan cihaza gönderir.
...
├── __init__.py        # kurulum, DataUpdateCoordinator, ham komut servisleri
├── api.py              # OAuth + kullanıcı adı/şifre girişi + tüm AC komutları
├── button.py             # kanatçık için hızlı salınım/durdur düğmeleri
├── climate.py              # climate entity (mod, fan hızı, sıcaklık, entity_picture)
├── config_flow.py           # kullanıcı adı/şifre (otomatik) + refresh_token (yedek)
├── const.py                  # endpoint'ler, sabit client_id/secret, kodlama sabitleri
├── select.py                  # dikey/yatay kanatçık pozisyonu (0-6)
├── sensor.py                   # tanı sensörleri (hata, hava kalitesi, filtre ömrü, ...)
├── switch.py                    # turbo/uyku/iyonizer/tasarruf + oto-kapatma aktif
├── time.py                       # otomatik kapatma hedef saati
├── assets/vestel_ac.png           # marka görseli (kurulumda www/'ye otomatik kopyalanır)
├── manifest.json
├── strings.json
